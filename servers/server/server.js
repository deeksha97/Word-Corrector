// imports
const { app: guiApp, BrowserWindow, ipcMain } = require('electron');
...
io.on('connection', (socket) => {
  ...
  socket.on('backup', ({ room }) => {
    backUpServerAddress = socket.id;
    guiWindow.webContents.send(
      'message',
      `>> info: connection with backup server established ${room}`
    );
    socket.join(room);
  });
  ...
  socket.on('misspledLexicon', ({ word }) => {
    ...
    if (!(word in misspelledWords)) {
      ...
      console.log(':::::::::::::::::::::::::', backUpServerAddress);
      io.to('backup').emit('misspledLexicon', { word });
    }
  });
